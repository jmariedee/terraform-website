...
Plans that contain no changes can be applied if you create a run in [No-Operation Apply Mode](/cloud-docs/run/modes-and-options#no-operation-apply). This is particularly useful for upgrading between Terraform versions in your workspace, [read more on upgrading workspace state.](/cloud-docs/workspaces/state#upgrading-state).  
