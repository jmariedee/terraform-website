...
A no-operation apply can be used to upgrade a Terraform Cloud workspace's latest state version to the desired Terraform version. Furthermore, no new configuration version needs to be uploaded in order to create a run in this mode.

- **API:** Set the `allow-empty-apply` field to `true`.
- **UI:** Use the "Start new plan" action from a workspace's "Actions" menu, then check the "Allow Empty Apply" box in the new plan dialog.
