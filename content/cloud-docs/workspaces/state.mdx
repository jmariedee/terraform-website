...
A workspace's state version can be upgraded to a new Terraform version using no-operation (empty) applies. Empty applies are a [type of run mode](/cloud-docs/run/modes-and-options) that allows Terraform Cloud to proceed with an apply when a plan produces no changes.   

-> **Note:** Upgrading workspace state is still subject to the same Terraform version constraints as when upgrading state locally. Please refer to the [Terraform upgrade guides](/language/upgrade-guides) for more information on upgrading between versions.
